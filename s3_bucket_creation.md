## Objective of this markdown is to explain:
* How to interact with S3 Programmatically using AWS CLI
* Create bucket 
* upload content to bucket
* download content from bucket
* configure a bucket to host the website and sync source file using AWS CLI


## Task 1: create a S3 bucket config file
```bash

# Step 1 : make a config file which contains the bucket name and other vairables
[S3]
bucket_name = notes-bucket-966252589
object_name = ./labRepo/notes
key_name = notes
...
# Task 2 : now create a file with the name create-bucket.py
```python 
...
def main(s3Client):
    ...
    print('Reading config file for the bucket name')
    config = readConfig()
    bucket_name = config['bucket_name']

    print('Verifying that bucket name is valid...')
    # verify that the bucket exists. The script with exit
    # if name name is not valid for new bucket
    verifyBucketName(s3Client, bucket_name)
    print(bucket_name)

    # Create bucket if it didn't exists
    createBucket(s3Client, bucket_name)

    # pause until the bucket is in the account
    print('\nConfirm that the bucket exists')
    verifyBucket(s3Client, bucket_name)

    print('\nEnd of the create Bucket Script')
